...
export default function QuestionSettings(props: propsType): JSX.Element {
    return (
        <div className={`${styles.mainContainer}`}>
            ...
            <div className={`${styles.categoryContainer} flex justify-center overflow-hidden mx-auto`}>
                {
                    categoriesList.map((item, index): JSX.Element => {
                        return(
                            <div key={index}><Btn onClick={() => props.setCategory('category=' + item.value)}>{item.title}</Btn></div>
                        )
                    })
                }
            </div>
            ...
            <div className={`${styles.difficultyContainer} flex justify-center overflow-hidden mx-auto mb-4`}>
                    <div><Btn onClick={() => props.setDifficulty('difficulty=easy')}>Easy</Btn></div>
                    <div><Btn onClick={() => props.setDifficulty('difficulty=medium')}>Normal</Btn></div>
                    <div><Btn onClick={() => props.setDifficulty('difficulty=hard')}>Hard</Btn></div>
            </div>
        </div>
    )
}